...
@plugin.command()
@lightbulb.option("farbe", "Die Farbe welche deine Rolle annehmen soll", type=str, default="000000")
@lightbulb.command("farbrolle", "Ändert deine Farbe")
@lightbulb.implements(lightbulb.PrefixCommand)
async def farbrolle(ctx: lightbulb.Context) -> None:
    ...
    if await permcheck(ctx):
        ...
        for role in rollen:
            if role.name == str(ctx.event.author_id):
                user_role = role
        
        if user_role is None:
            new_role = await ctx.bot.rest.create_role(
                ctx.guild_id,
                name=str(ctx.event.author_id),
                permissions=(
